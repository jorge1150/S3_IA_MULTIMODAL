...
Ejecutar una vez para identificar los índices de micrófono y altavoz.
...
def probar_microfono(indice: int, duracion: int = 2):
    """Graba 2 segundos del micrófono y muestra el nivel de volumen."""
    ...
    try:
        ...
        print(f"  Índice [{indice:2d}] | Volumen: {volumen:.4f} | {barra:<50} | {estado}")
        return volumen
    except Exception as e:
        print(f"  Índice [{indice:2d}] | ERROR: {e}")
        return 0.0
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            idx = int(sys.argv[1])
            probar_microfono(idx)
        except ValueError:
            print("Uso: python audio/scanner.py [índice_del_micrófono]")
    else:
        print("\nUso: python audio/scanner.py <índice> para probar un micrófono específico.")
        print("Luego actualiza MIC_DEVICE_INDEX en config.py con el índice correcto.")
